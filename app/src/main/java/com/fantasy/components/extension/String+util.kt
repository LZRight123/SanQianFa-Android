...
// HORIZONTAL ELLIPSIS (…)
fun String.truncate(count: Int, truncate: String = "\u2026"): String =
    if (trim().count() > count) {
        trim().substring(0, count) + truncate
    } else {
        trim()
    }

fun String.touMing() = replace(
    "(\\d{3})\\d{4}(\\d{4})".toRegex(),
    "$1****$2"
)

fun String.placeholder(holder: String = "-"): String = ifBlank { holder }
...
/**
 * "naduo://activity/clickLink?title=小在&TYPE=1&url=https%3A%2F%2Fliangmc.com%2Ftest%3Fkey1%3D1%26key2%3D2"
 * title -> 小在
 * TYPE -> 1
 * url -> https://liangmc.com/test?key1=1&key2=2
 */
fun String.queryItems(): Map<String, String?> {
 ...
 * 限定用户史
 */
fun String.verifyUsername(): String? {
    val allowedCharacterSet = ('0'..'9') + ('a'..'z') + ('A'..'Z') + ('\u4E00'..'\u9FFF') // 允许的字符集合
    val usernameInputCharacterSet = toSet()
    // 检查是否包含不允许的字符并且用户名长度小于12
    return if (usernameInputCharacterSet.all { it in allowedCharacterSet }) {
        take(12)
    } else {
        null
    }
